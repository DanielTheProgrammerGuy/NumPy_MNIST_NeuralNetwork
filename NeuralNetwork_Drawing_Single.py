...
while True:
    ...
    xstart = np.where(np.sum(screen_array,axis = 1)>0)[0][0]
    ystart = np.where(np.sum(screen_array,axis = 0)>0)[0][0]
    xend = np.where(np.sum(screen_array,axis = 1)>0)[0][-1]+1
    yend = np.where(np.sum(screen_array,axis = 0)>0)[0][-1]+1
    ...
    data_raw[
        ...
        ] = screen_array[xstart:xend,ystart:yend]
    ...
    for i in range(28):
        for j in range(28):
            ...
            colour_value = int(255 * data_processed[j, i])
            pg.draw.rect(screen, (colour_value, colour_value, colour_value), pg.Rect(i * 10, j * 10, 10, 10))

    data_processed = data_processed.flatten()
    guess_output = nn.feedforward([data_processed])
    guess = np.argmax(guess_output)

    pg.draw.rect(screen,(0,255,0),(260,0,20,20),3)
    number = font.render(str(guess), True, (0, 255, 0))
    number_rect = number.get_rect(center=(270,10))
    screen.blit(number,number_rect)

    pg.draw.rect(screen, (0, 255, 0), (0, 0, 60, 20), 3)
    certainty = font.render(f"{(2*guess_output[0,guess]-np.sum(guess_output))*100:.2f}%", True, (0, 255, 0))
